        ...
        if (file_ok(fhandle))
        {
            ...
            if (start+size>=fhandle->ptr->size)
            {
                ...
                if (neededblocks>totalblocks) //more blocks needed so we request for one
                {
                    ...
                    {
                        #ifdef WRITE_DEBUG
                        printf("vfs_directwrite(): Adding sectors to file\n");
                        ...
                        #ifdef WRITE_DEBUG
                        printf("vfs_directwrite(): Adding sectors done.\n");
                        #endif            
                        if (res == -1) return 0;
                    }
                    else
            ...
            #ifdef WRITE_DEBUG
            printf("vfs_directwrite(): Telling fs to write the file\n");
            #endif            
            ...
            #ifdef WRITE_DEBUG
            printf("vfs_directwrite(): write done.\n");
            #endif            
...
int vfs_mount_device(const char *fsname,const char *devname,const char *location)
{
    ...
    else
    {
        ...
        #ifdef DEBUG_MOUNT
        printf("calling fs mount function..\n");
        #endif
...
file_PCB *openfilex(char *filename,int mode)
{
    ...
    vfs_setbuffer(fcb,0,1024,FILE_IOFBF);
    ...
    if (fhandle!=0)
        if (file_ok(fhandle))
        {
            if (size>FILE_BUFSIZE)
            {
                ...
                #ifdef WRITE_DEBUG
                    printf("vfs_directwrite(): called.\n");
                #endif
                
                retval =  vfs_directwrite(buf,itemsize,n,fhandle);
                
                #ifdef WRITE_DEBUG
                    printf("vfs_directwrite(): finished.\n");
                #endif
                return retval;
            };
            ...
            for (i=0;i<size;i++)
            {
                ...
                #ifdef WRITE_DEBUG
                    printf("vfs_writechar(): called.\n");
                #endif

                if (vfs_writechar(fhandle,buf[i])!=1) break;       
                
                #ifdef WRITE_DEBUG
                    printf("vfs_writechar(): finished.\n");
                #endif
