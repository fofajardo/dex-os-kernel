...
void readBPB(BPB *bpbblock,int id)
{
   ...
   while (1)
   {
       if (dex32_IOcomplete(handle)) break;
   };
   dex32_closeIO(handle);
};
...
int fat_getfreeblocks(int id)
{
   ...
   for (i=0; i < bpbblock.total_sectors - 3; i++)
      if (obtain_next_cluster(i,fat,fat_type,&bpbblock,id)==0) total++;
...
DWORD fat_addsectors(vfs_node *f, DWORD sectors /*sectors to add*/,int id)
{
   fatdirentry *pdir;
   ...
   cur = getdirsectorsize(desc,&bpbblock,1,fat,id); 
   ...
   for (i=0;i<sectors;i++)
   {
      ...
      fc = obtainfreecluster(fat,bpbblock.total_sectors,&bpbblock,fat_type,id);
...
DWORD fat_deletefile(vfs_node *f,int id)
{
   ...
   cluster = obtain_next_cluster(desc->st_clust,fat,fat_type,&bpbblock,id);
   if (desc->st_clust!=0)
   {
      ...
      while (cluster < fat_get_eoc(fat_type))
      {
         DWORD next_cluster = obtain_next_cluster(cluster, fat, fat_type, &bpbblock, id);
   ...
   for (i=0;i<maxindex;i++)
   {
      ...
      dir[i].name[0]==0x00)
      {
         ...
         if (f->attb&FILE_DIRECTORY)
         {
            ...
            dir[i].st_clust = fc;
            f->start_sector = fc;
            dirent = (fatdirentry*) malloc(dirsize);
            f->misc2 = (void*)dirent;
            f->miscsize2 = dirsize;
            memset(f->misc2,0,dirsize);
            ...
            writefile12EX2(f->misc,bpbblock,dirent,1,0,dirsize,id);
         };
...
int obtainfreecluster(BYTE *fat,int maxentries, BPB *bpbblock, int fat_type,int id)
{
   int i;
   
   for (i=0; i < maxentries; i++)
   {
      if (obtain_next_cluster(i, fat, fat_type, bpbblock, id) == 0) return i;
   };
...
int writefile12EX2(fatdirentry *dir, BPB *bpbblock, char *buf, int se, int start, int end, int id)
      {
         ...
         if (fat_type!=FAT12_FAT32)
         {
             ...
             fat = (BYTE*)malloc(bpbblock->sectors_per_fat*512);
             loadfat(bpbblock,fat,id);
         };
         ...
         do {
            ...
            if (block >= startblock && block <= endblock)
            {
               ...
               //printf("fat: writing to cluster %d(%d)\n",cluster,sector);
