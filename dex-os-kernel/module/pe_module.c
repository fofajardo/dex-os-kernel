...
/*Call entrypoint of a library or device driver module*/
int module_call_library(void *entrypoint, lib_PCB *lib_info, char *parameters)
{
  int (*libentrypoint)(devmgr_generic *ptr,char *p);
  char temp[255];
  
  /*The device manager interface, is passed as a parameter to the
    library or device driver entrypoint*/
  ...
  if (libentrypoint(interface,parameters) == -1) 
      ...
      printf("pe_module: Library or Driver returned with error.\n");
      //return with error
      return -1; 
      };
      
  printf("pe_module: Call successful.\n");
  return 0;
};
        ...
        ) //location of image to load
{
  ...
  // printf("dex32_module() called..\n");
   ...
   //this should always be the last
      ...
      if (libentrypoint!=0)
      {
		if (module_call_library(libentrypoint, libptr, p)==-1) return -1;
      };
   ...
   if (mode==PE_USEREXE || mode==PE_SYSEXE)
    {
             ...
             if (entrypoint==0)
             {
                   printf("pe_module: Calling Standard entrypoint..\n");
                   entrypoint=(void*)(base+opthdr->AddressOfEntryPoint);
             }
                  else
             {
                   printf("pe_module: Calling dex32_main()..\n");       
             };     
