...
int ide_uni_get_bytes_per_block()
{
     ...
     for (i=0; i < 4; i++)
      ...
      if (info_index == -1) 
      {
          printf("ATA driver: unidentified device id (%d) supplied!\n",devmgr_getcontext());
          return -1;
      };
...
int ide_uni_read_block(int block,char *blockbuff,int numblocks)
{
 int info_index = -1;
 int i;
	printf("ide_uni_read_block called, service block %d\n",block);
 /* get the index of the drive information structure associated with this device
    depending on which device DEX used to call this function, this is done using
    the context inforation set by the bridge when interfacing with this module*/
 for (i=0; i < 4; i++)
        ...
        if (mbr->tables[i].type!=0)
           {
			    ...
			    mypartition.get_block_size = ide_uni_get_bytes_per_block;
              ...
              if (p!=0)
                   {
                         ...
                         if (strcmp(p,"-stat")==0)
                             ...
                             printf("Heads        : %d\n", ide_drivelist[info].heads);
                             printf("Cylinders    : %d\n", ide_drivelist[info].cylinder);
                             printf("SPT          : %d\n", ide_drivelist[info].sectors_per_track);
                             printf("Bytes/Sector : %d\n", ide_drivelist[info].bytes_per_sector);
                             }
                         else
                         if (strcmp(p,"-ver")==0)
                             {
